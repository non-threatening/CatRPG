...
#
#
#func _unhandled_input(event: InputEvent) -> void: ##interact with dialog system when its active
	#if is_active == false: # only runs when dialog open.
		#return
	#if(
	...
	#):
		#if text_in_progress == true: ## Skip the typing out and show all text at once
...
#func show_dialog( _items : Array[ DialogItem ] ) -> void:
	...
	#
	#
#func hide_dialog() -> void: # disabling dialog ui node, not the system
	#is_active = false
...
#func start_dialog() -> void:
	...
	#
#
### Set text, then start typing timer
...
#
#
### Set dialog choices in UI, buttons
...
#func start_timer() -> void:
	...
	## manipulate text speed, wait_time
