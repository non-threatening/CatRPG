...
@onready var right_stick_panel_container: TextureRect = $Bubble/RightStickPanelContainer
...
func _ready() -> void:
	...
	SignalBus.freq_tut_show_stick.connect( _right_stick_panel_container )
	right_stick_panel_container.hide()
...
func _right_stick_panel_container( freq_tut_triggers ) -> void:
	match freq_tut_triggers:
		"show":
			right_stick_panel_container.show()
			var tween_show : Tween = create_tween()
			tween_show.set_pause_mode( Tween.TWEEN_PAUSE_PROCESS )
			tween_show.set_ease( Tween.EASE_IN )
			tween_show.tween_property( right_stick_panel_container, "modulate", Color( 1, 1, 1, 1 ), 0.666 )
		"hide":
			var tween_hide : Tween = create_tween()
			tween_hide.set_pause_mode( Tween.TWEEN_PAUSE_PROCESS )
			tween_hide.set_ease( Tween.EASE_IN )
			tween_hide.tween_property( right_stick_panel_container, "modulate", Color(1.0, 1.0, 1.0, 0.0), 0.666 )
		_:
			pass
...
func apply_dialogue_line() -> void:
	...
	if not dialogue_line.tags.is_empty():
		tut_img = ( "_" + dialogue_line.get_tag_value("tut_img") )
		prints("tut)img:", tut_img + ".png")
