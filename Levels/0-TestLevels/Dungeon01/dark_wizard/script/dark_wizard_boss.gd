...
const ENERGY_EXPLOSION_SCENE : PackedScene = preload( "res://Levels/0-TestLevels/Dungeon01/dark_wizard/EnergyExplosion.tscn" )
const ENERGY_BALL_SCENE : PackedScene = preload( "res://Levels/0-TestLevels/Dungeon01/dark_wizard/energy_orb.tscn" )

@export var max_hp : int = 10
var hp : int = 10

var audio_hurt : AudioStream = preload("res://Levels/0-TestLevels/Dungeon01/dark_wizard/audio/boss_hurt.wav")
var audio_shoot : AudioStream = preload("res://Levels/0-TestLevels/Dungeon01/dark_wizard/audio/boss_fireball.wav")
...
@onready var door_block: LevelTileMap = $"../DoorBlock"
...
func _ready() -> void:
	...
	if persistant_data_handler.value == true: #if boss defeated it = true
		door_block.enabled = false
		queue_free()
		return
	
	hp = max_hp
	PlayerHud.boss_show_health( "Bad Guy" )
...
func idle() -> void:
	...
	while _t == current_position: #pick rand numbers until it's not the current one
		_t = randi_range( 0, 3 )
	teleport( _t )
	pass
...
func explosion( _p : Vector2 = Vector2.ZERO ) -> void:
	...
	get_parent().add_child.call_deferred( e ) # Add explosion to the scene
	pass
